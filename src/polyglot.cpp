...
OpeningBook book;
string openingBookPath = "<empty>";

 U64 polyglotPieces[768]  = { 0x9D39247E33776D41ULL, 0x2AF7398005AAA5C7ULL, 0x44DB015024623547ULL,
...
U64 polyglotCastle[4] = {0x31D71DCE64B2C310ULL, 0xF165B587DF898190ULL, 0xA57E6339DD2CF3A0ULL, 0x1EF6E6DBB1961EC9ULL};
U64 polyglotEP[8] = {0x70CC73D90BC26E24ULL, 0xE21A6B35DF0C3AD7ULL, 0x003A93D8B2806962ULL, 0x1C99DED33CB890A1ULL, 0xCF3145DE0ADD4289ULL, 0xD0E4427A5514FB72ULL, 0x77C621CC9FB3A483ULL, 0x67A34DAC4356550BULL};
U64 polyglotTurn = 0xF8D626AAAF278509ULL;
...
void OpeningBook::init(string &path)
{
    if (path.length() == 0)
    {
        bookEnabled = false;
    ...
    bookArray = new PolyglotEntry[numEntries];

    size_t sizeCheck = fread(bookArray, 1, filesize, file);
    ...
    if (sizeCheck > 0)
    {
        for (int i = 0; i < numEntries; i++)
            correctEndian(&bookArray[i]);

        bookEnabled = true;
    }
}

void OpeningBook::set_use_best(bool useBest)
{
    use_best_move = useBest;
}

void OpeningBook::set_max_depth(int depth)
{
    max_depth = depth;
}

void OpeningBook::correctEndian(PolyglotEntry *entry)
{
    if (isLittleEndian)
    {
        entry->key = swap_uint64(entry->key);
        entry->bestMove = swap_uint16(entry->bestMove);
...
bool OpeningBook::is_little_endian()
{
    int num = 1;
...
uint64_t OpeningBook::swap_uint64(uint64_t d)
{
    uint64_t a;
...
uint32_t OpeningBook::swap_uint32(uint32_t d)
{
    uint32_t a;
    uint8_t *dst = (uint8_t *)&a;
    uint8_t *src = (uint8_t *)&d;
...
uint16_t OpeningBook::swap_uint16(uint16_t d)
{
    uint16_t a;
    uint8_t *dst = (uint8_t *)&a;
    uint8_t *src = (uint8_t *)&d;
...
U64 OpeningBook::polyglotKey(const Position & pos)
{
    U64 key = 0;
    U64 b = pos.occupiedBB[0] | pos.occupiedBB[1];

    while (b)
    {
        int s = popLsb(&b);
        PieceCode p = pos.mailbox[s];
        key ^= polyglotPieces[(2 * ((p >> 1) - 1) + ((p & SIDESWITCH) == WHITE))*64 + s];
    }

    if (pos.castleRights & WKCMASK)  key ^= polyglotCastle[0];
    if (pos.castleRights & WQCMASK) key ^= polyglotCastle[1];
    if (pos.castleRights & BKCMASK)  key ^= polyglotCastle[2];
    if (pos.castleRights & BQCMASK) key ^= polyglotCastle[3];

    if (pos.epSquare != 0)
        key ^= polyglotEP[FILE(pos.epSquare)];
...
Move OpeningBook::polyglot_to_move(const Position & pos, unsigned short pg_move)
{
    Move move = Move(pg_move);

    int pt = (move >> 12) & 7;
    if (pt)
        return makePromotionMove(from_sq(move), to_sq(move), PieceType(pt + 1), PROMOTION);
...
int OpeningBook::find_first_key(U64 key)
{
    firstEntry = -1;
    bestEntry = -1;
    randomEntry = -1;
    ...
    {
        if (key == bookArray[i].key)
        {
            firstEntry = i;
            while ((firstEntry>0) && (key == bookArray[firstEntry - 1].key))
                firstEntry--;
            return get_key_data();
        }
    }
...
int OpeningBook::get_key_data()
{
    int best_weight = bookArray[firstEntry].weight;
    int totalWeight = best_weight;
    U64 key = bookArray[firstEntry].key;

    int numHits = 1;
    bestEntry = firstEntry;

    for (int i = firstEntry + 1; i<numEntries; i++)
    {
        if (bookArray[i].key != key)
            break;

        numHits++;
        totalWeight += bookArray[i].weight;
        if (bookArray[i].weight > best_weight)
        {
            best_weight = bookArray[i].weight;
            bestEntry = i;
        }
    }

    int randomWeight = (randomGen->rand() % totalWeight);
    int weight_count = 0;
    randomEntry = bestEntry;

    for (int i = firstEntry; i < firstEntry + numHits; i++)
    {
        if ((randomWeight >= weight_count) && (randomWeight < weight_count + bookArray[i].weight))
        {
            randomEntry = i;
            break;
        }
        weight_count += bookArray[i].weight;
    }

    return numHits;
}

bool OpeningBook::check_do_search(const Position & pos)
{
    currentPosition = pos.occupiedBB[0] | pos.occupiedBB[1];
    currentCount = POPCOUNT(currentPosition);

    bool pos_changed = false;

    U64 b = currentPosition ^ lastPosition;
    int n2 = POPCOUNT(b);

    if (n2 > 6) pos_changed = true; // If there are more than 6 changed pieces
    if (currentPosition == lastPosition) pos_changed = true;
    if (currentCount > lastCount) pos_changed = true; // Position must have been reset by GUI
    if (currentCount < lastCount - 2) pos_changed = true; // Only up to two pieces may leave the board in one full turn
    if (pos.key == 8616220903129209699) pos_changed = true; // If Position is starting position again
...
Move OpeningBook::probe(Position& pos)
{
    ...
    if (!bookEnabled) return m1;
    if (!check_do_search(pos)) return m1;

    if (plyUsed >= max_depth)
        return m1;
    ...
    m1 = polyglot_to_move(pos, bookArray[idx1].bestMove);
    return m1;
}
